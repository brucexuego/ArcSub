...
const LOCAL_TRANSLATION_MODEL_PROFILES: LocalTranslationModelProfile[] = [
  ...
  },
  {
    ...
    notes: ['Qwen3 4B plain_probe can use larger line-safe batches after local benchmark validation.', 'Validated mode uses moderate line-safe batches; marker and line-count checks recursively split unsafe chunks.', 'JSON strict mode uses small line-safe batches while keeping marker and line-count validation active.'],
  },
  {
    ...
    notes: ['TranslateGemma plain_probe uses shared line-safe markers and can run larger batches after local benchmark validation.', 'Validated mode uses moderate contiguous batches; coverage-loss detection recursively splits unsafe chunks.', 'JSON strict mode stays single-line for deterministic repair fallbacks.'],
  },
  {
